!(function (t) {
    ...
    var n = i(2),
      ...
      s = (function () {
        function t(t) {
          ...
          (this.$$callback = t), r.push(this);
        }
        return (
          (t.prototype.observe = function (t) {
            var e = a("observe", t);
            if (e) throw TypeError(e);
            c(this.$$observationTargets, t) > 0 ||
              (this.$$observationTargets.push(new n.ResizeObservation(t)), f());
          }),
          (t.prototype.unobserve = function (t) {
            var e = a("unobserve", t);
            if (e) throw TypeError(e);
            var i = c(this.$$observationTargets, t);
            i < 0 || (this.$$observationTargets.splice(i, 1), m());
          }),
          ...
          r.forEach(function (e) {
            if (e.$$activeTargets.length) {
              ...
              e.$$activeTargets.forEach(function (e) {
                ...
                i.push(n),
                  (e.$$broadcastWidth = n.contentRect.width),
                  (e.$$broadcastHeight = n.contentRect.height);
                var r = h(e.target);
                r < t && (t = r);
              }),
                e.$$callback(i, e),
  ...
  function (t, e, i) {
    ...
    const l = new Promise(async (t, e) => {
      c && t();
      const i = async () => {
        (c = await window.loadCardHelpers()), (window.cardHelpers = c), t();
      };
      window.loadCardHelpers
        ...
        : window.addEventListener("load", async () => {
            !(function () {
              ...
              if (((t.hass = s()), !t.hass || !t.hass.panels)) return !1;
              (t.route = { path: "/lovelace/" }), t._updateRoutes();
              try {
                document.querySelector("home-assistant").appendChild(t);
              } catch (t) {
              } finally {
                document.querySelector("home-assistant").removeChild(t);
              }
    ...
    function d(t, e) {
      ...
      return (
        customElements.whenDefined("hui-error-card").then(() => {
          ...
          t.setConfig(i), n.parentElement && n.parentElement.replaceChild(t, n);
        }),
        l.then(() => {
          a("ll-rebuild", {}, n);
        }),
    ...
    function u(t, e) {
      if (!e || "object" != typeof e || !e.type)
        return d(`No ${t} type configured`, e);
      let i = e.type;
      if (
      ...
      const n = d(`Custom element doesn't exist: ${i}.`, e);
    ...
    const h = (t) =>
        "function" == typeof t.getCardSize
          ? t.getCardSize()
          : customElements.get(t.localName)
          ? 1
          : customElements.whenDefined(t.localName).then(() => h(t)),
      p = async (t, e, i) => {
        const n = (t) =>
          "string" == typeof t && t.endsWith("%")
            ? Math.floor((e * parseInt(t)) / 100)
            : parseInt(t);
        let o = 0;
        if ("object" == typeof i.column_width) {
          let t = e;
          for (; t > 0; ) {
            let e = i.column_width[o];
            void 0 === e && (e = i.column_width.slice(-1)[0]),
              (t -= n(e)),
              (o += 1);
          }
          o = Math.max(o - 1, 1);
        } else o = Math.floor(e / n(i.column_width));
        (o = Math.max(o, i.min_columns)),
          (o = Math.min(o, i.max_columns)),
          "auto" === i.layout &&
            "docked" === s().dockedSidebar &&
        ...
        switch (i.layout) {
          case "horizontal":
            await (async (t, e, i) => {
              ...
              for (const i of t) {
                if (((n += 1), !i)) continue;
                const t = e[(n - 1) % e.length];
                t.appendChild(i), (t.length += await h(i));
              }
            })(t, r);
            break;
          case "vertical":
            ...
            await (async (t, e, i) => {
              function n() {
                ...
                for (let n = 0; n < e.length; n++) {
                  if (e[n].length < i.min_height) return n;
        ...
        return (r = r.filter((t) => t.childElementCount > 0)), r;
      };
    var f = i(1);
    class g extends n {
      ...
      async setConfig(t) {
        (this._config = {
          ...
          min_height: 5,
          column_width: 300,
          max_width: t.column_width || "500px",
          min_columns: t.column_num || 1,
          max_columns: t.column_num || 100,
          sidebar_column: !1,
          ...t,
        }),
          (this.cards = []),
      ...
      connectedCallback() {
        ...
        let t = this.parentElement,
          e = 10;
        for (; e-- && t; ) {
          if ("HUI-PANEL-VIEW" === t.tagName) this.classList.add("panel");
          else if ("HUI-VERTICAL-STACK-CARD" === t.tagName)
            this.classList.add("stacked");
          else if ("DIV" !== t.tagName && "root" !== t.id) break;
          t = t.parentElement ? t.parentElement : t.getRootNode().host;
        }
      }
      async firstUpdated() {
        window.addEventListener("location-changed", () => {
          "" === location.hash && setTimeout(() => this.updateSize(), 100);
        }),
          this.resizer ||
            ((this.resizer = new f.ResizeObserver(() => {
              this.updateSize();
            })),
      ...
      async updateSize() {
        ...
        this.classList.contains("panel") &&
          ...
          (this.hass && "docked" === this.hass.dockedSidebar
            ? (t += 256)
            : (t += 64)),
          t &&
            Math.abs(t - this._layoutWidth) > 50 &&
            ((this._layoutWidth = t),
      ...
      async updated(t) {
        ...
        if (t.has("hass") && this.hass && this.cards)
          for (const t of this.cards) t && (t.hass = this.hass);
      }
      async build_card(t) {
        if ("break" === t) {
          if ("grid" === this._config.layout) {
            const t = document.createElement("div");
            return this.shadowRoot.querySelector("#staging").appendChild(t), t;
          }
          return null;
        }
        const e = { ...t, ...this._config.card_options },
          i = (function (t) {
            return c ? c.createCardElement(t) : u("card", t);
          })(e);
        return (
          (i.hass = s()),
          "grid" === this._config.layout &&
            ((i.style.gridColumn = e.gridcol || "auto"),
            (i.style.gridRow = e.gridrow || "auto"),
            (i.style.gridArea = e.gridarea || "'myArea'"),
            (i.style.alignSelf = e.align_self || "'auto'"),
            (i.style.justifySelf = e.justify_self || "'auto'")),
          this.shadowRoot.querySelector("#staging").appendChild(i),
          new Promise((t, e) =>
            i.updateComplete ? i.updateComplete.then(() => t(i)) : t(i)
          )
        );
      }
      async build_cards() {
        const t = this.shadowRoot.querySelector("#staging");
        for (; t.lastChild; ) t.removeChild(t.lastChild);
        return Promise.all(
          (this._config.entities || this._config.cards).map((t) =>
            this.build_card(t)
          )
        );
      }
      async place_cards() {
        "grid" !== this._config.layout &&
          this.cards.length &&
          ((this.columns = await p(
            this.cards,
            this._layoutWidth || 1,
            this._config
          )),
          this._config.rtl && this.columns.reverse(),
          this.format_columns());
      }
      format_columns() {
        const t = (t, e, i, n = "px") => {
          if (void 0 === this._config[e]) return "";
          let o = t + ": ";
          ...
          return (
            "object" == typeof r
              ? r.length > i
                ...
                : (o += "" + r.slice(-1))
              : (o += "" + r),
            o.endsWith("px") || o.endsWith("%") || (o += n),
            o + ";"
          );
        };
        for (const [e, i] of this.columns.entries()) {
          const n = [
            t("max-width", "max_width", e),
            t("min-width", "min_width", e),
            t("width", "column_width", e),
            t("flex-grow", "flex_grow", e, ""),
          ];
          i.style.cssText = "".concat(...n);
        }
      }
      getCardSize() {
        return this.columns && this.columns.length
          ? Math.max.apply(
              Math,
              this.columns.map((t) => t.length)
            )
          : this._config.entities
          ? 2 * this._config.entities.length
          : this._config.cards
          ? 2 * this._config.cards.length
          : 1;
      }
      render() {
        return "grid" === this._config.layout
          ? o`
        ...
        style="
          ...
          --gridareas_large: ${this._config.gridareas};
    ...
    if (!customElements.get("layout-card-griddier")) {
